...
- [x] 默认本地观测不新增依赖，现有 `requirements.txt` 足以运行 Span、Metrics、脱敏和 JSON Lines 导出。
- [x] OTLP 导出为可选项，明确声明 `opentelemetry-api`、`opentelemetry-sdk` 与 `opentelemetry-exporter-otlp-proto-http`；依赖或导出失败不阻塞或改变 Agent 执行结果。
...
## 验证
- [x] 单元测试覆盖配置、Span 上下文、树结构、指标、本地导出、OTLP 失败隔离和成本计算。
